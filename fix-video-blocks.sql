-- Fix Video Blocks Support
-- This script adds video support to session_notes_blocks and updates the function
...
-- 4. Safely update the block_type constraint
DO $$
DECLARE
...
BEGIN
    ...
    -- Check if 'video' already exists in the data
    SELECT EXISTS(SELECT 1 FROM session_notes_blocks WHERE block_type = 'video') INTO has_video;
    
    -- Create new constraint based on what exists
    IF has_video THEN
        -- If video already exists, allow text, drill, and video
        ALTER TABLE session_notes_blocks 
        ADD CONSTRAINT session_notes_blocks_block_type_check 
        CHECK (block_type IN ('text', 'drill', 'video'));
        RAISE NOTICE 'Created constraint allowing: text, drill, video';
    ELSE
        -- If no video exists yet, allow text, drill, and video
        ALTER TABLE session_notes_blocks 
        ADD CONSTRAINT session_notes_blocks_block_type_check 
        CHECK (block_type IN ('text', 'drill', 'video'));
        RAISE NOTICE 'Created constraint allowing: text, drill, video';
...
-- 5. Update the get_session_with_planning function to include video data
...
-- 6. Update the save_session_planning function to handle video blocks
