-- Fix Superadmin Access for Session Management (Functions Only)
...
-- 2. Create a new helper function to check if user is superadmin (avoiding conflicts with existing function)
CREATE OR REPLACE FUNCTION check_user_superadmin(user_uuid UUID)
RETURNS BOOLEAN AS $$
BEGIN
    RETURN EXISTS (
...
-- 3. Update the save_session_planning function to allow superadmin access
...
BEGIN
    -- Check if user is superadmin using the new function
    SELECT check_user_superadmin(auth.uid()) INTO is_user_superadmin;
...
-- 4. Update the get_session_with_planning function to also allow superadmin access
...
-- 5. Show the current roles
SELECT 'Current Roles in System:' as info;
SELECT id, name, description FROM roles ORDER BY name;

-- 6. Show any existing superadmin users
SELECT 'Existing Superadmin Users:' as info;
SELECT 
    ur.user_id,
    r.name as role_name
FROM user_roles ur
