/**
 * Calculate player statistics for a single game session
 * This function extracts the exact logic from GameStats.jsx to ensure consistency
 */
export function calculatePlayerGameStatsCorrect(player, gameEvents, gameSession) {
  ...
  for (const goalEvent of goalEvents) {
    ...
    // Debug: Show detailed goal event processing for Kael
    if (player.id === '8cdeac25-6589-4a4d-9c22-3b2d28508e0e') { // Kael Telfer's ID
      console.log(`Kael - Processing goal event:`, {
        id: goalEvent.id,
        type: goalEvent.event_type,
        time: goalEvent.event_time,
    ...
    if (wasOnRinkAtGoal) {
      if (goalEvent.event_type === 'goal_for') {
        plusMinus += 1
        // Debug: Log goal FOR for Kael
        if (player.id === '8cdeac25-6589-4a4d-9c22-3b2d28508e0e') { // Kael Telfer's ID
          console.log(`Kael - Goal FOR: +1, running total: ${plusMinus}`)
        }
      } else if (goalEvent.event_type === 'goal_against') {
        plusMinus -= 1
        // Debug: Log goal AGAINST for Kael
        if (player.id === '8cdeac25-6589-4a4d-9c22-3b2d28508e0e') { // Kael Telfer's ID
          console.log(`Kael - Goal AGAINST: -1, running total: ${plusMinus}`)
        }
      }
    } else {
      // Debug: Log when Kael was not on rink
      if (player.id === '8cdeac25-6589-4a4d-9c22-3b2d28508e0e') { // Kael Telfer's ID
        console.log(`Kael - Goal ${goalEvent.event_type} but NOT on rink: no change, running total: ${plusMinus}`)
      }
    }
  }
