...
-- Create functions to help with media management
CREATE OR REPLACE FUNCTION get_drill_media(drill_uuid UUID)
RETURNS TABLE (
    media_id UUID,
    title TEXT,
    description TEXT,
...
BEGIN
    ...
    WHERE dm.drill_id = drill_uuid
    ORDER BY ma.created_at DESC;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

CREATE OR REPLACE FUNCTION get_session_media(session_uuid UUID)
RETURNS TABLE (
    media_id UUID,
    title TEXT,
    description TEXT,
...
BEGIN
    ...
    SELECT 
        ...
        ma.created_at
    FROM media_attachments ma
    INNER JOIN session_media sm ON ma.id = sm.media_id
