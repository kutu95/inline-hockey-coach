...
-- 2. Create session_template_blocks table for structured template content
...
CREATE POLICY "Users can view session templates" ON session_templates
  FOR SELECT USING (
    ...
    (organization_id IS NOT NULL AND EXISTS (
      ...
      AND ur.organization_id = session_templates.organization_id
  ...
  FOR SELECT USING (
    EXISTS (
      ...
      AND (
        ...
        OR
        (st.organization_id IS NOT NULL AND EXISTS (
          SELECT 1 FROM user_roles ur 
          WHERE ur.user_id = auth.uid() 
          AND ur.organization_id = st.organization_id
          AND ur.role IN ('coach', 'admin', 'superadmin')
        ))
      )
    )
...
-- 7. Create function to update updated_at column for session_templates
...
-- 8. Create trigger for session_templates updated_at
...
-- 9. Create function to update updated_at column for session_template_blocks
...
-- 10. Create trigger for session_template_blocks updated_at
...
-- 11. Create function to get template with blocks
...
-- 12. Create function to save template with blocks
